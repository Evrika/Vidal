...
class DCommand extends ContainerAwareCommand
{
	...
	private function sendToAll($output, $delivery)
	{
		...
		$template1 = $templating->render('VidalMainBundle:Delivery:d_delivery_start.html.twig', array('delivery' => $delivery));

		# рассылка
		for ($i = 0; $i < count($users); $i++) {
			$template2 = $templating->render('VidalMainBundle:Delivery:d_delivery_end.html.twig', array('delivery' => $delivery, 'user' => $users[$i]));
	...
	private function sendTo(array $emails, $delivery, $local = false)
	{
		...
		foreach ($users as $user) {
			$template2 = $templating->render('VidalMainBundle:Delivery:d_delivery_end.html.twig', array('delivery' => $delivery, 'user' => $user));
			$template  = $template1 . $template2;
